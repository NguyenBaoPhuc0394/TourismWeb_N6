...
namespace TourismWeb.Services.Implementations
{
    public class BookingService : IBookingService
    {
        ...
        public async Task<IEnumerable<BookingDataTableDTO>> GetAllBookingDataTable()
        {
            var result = await _bookingRepository.GetAllBookings();
            return _mapper.Map<IEnumerable<BookingDataTableDTO>>(result);
        }

        public async Task<bool> UpdateStatus(BookingUpdateDTO data)
        {
            var result = await _bookingRepository.UpdateStatus(data);
            return result;
        }
    }
}
