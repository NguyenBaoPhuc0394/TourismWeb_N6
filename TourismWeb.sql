...
BEGIN
    ...
    SELECT @MaxId = MAX(Id) FROM Categories;
    IF @MaxId IS NULL
        SET @NewId = 'CATE001';
    ELSE
        SET @NewId = 'CATE' + RIGHT('000' + CAST(CAST(SUBSTRING(@MaxId, 5, 3) AS INT) + 1 AS VARCHAR), 3);

    INSERT INTO Categories (Id, Name, Description)
    VALUES (@NewId, @Name, @Description);

    SELECT * FROM Categories WHERE Id = @NewId;
END
GO
...
BEGIN
	...
	IF @MaxId IS NULL
		SET @NewId = 'LOC001';
	ELSE
		SET @NewId = 'LOC' + RIGHT('000' + CAST(CAST(SUBSTRING(@MaxId, 4, 3) AS INT) + 1 AS VARCHAR), 3);

	INSERT INTO Locations (Id, Name)
	VALUES (@NewId, @Name);

	SELECT * FROM Locations WHERE Id = @NewId;
END
...
Select c.Id from Accounts a inner join Customers c on a.Email = c.Email where a.Username = 'customer'

SELECT b.Id,Booking_date, Number_of_adult_bookings, Number_of_children_bookings, Total_price 
FROM Bookings b inner join Customers c on b.Customer_Id = c.Id
where c.Id = 'CUS001'

SELECT DISTINCT t.Name, s.Start_date
FROM Tours t
INNER JOIN Schedules s ON t.Id = s.Tour_Id
INNER JOIN Bookings b ON s.Id = b.Schedule_Id
where b.Id = 'BOOK001'



update Bookings set status = 2 where id = 'BOOK003'
